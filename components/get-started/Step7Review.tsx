...
export function Step7Review({ data, set }: Props) {
  const displayName =
  ...
  const serviceLabels =
    data.selectedServices
      ?.map((s) => {
        ...
        if (s === "renewable-energy") return "Renewable Energy Solutions";
        return s;
      })
      .join(", ") || "None selected";
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-2">
          <div className="col-span-2">
            <SummaryItem label="Selected Services" value={serviceLabels} />
          </div>
          {isRealEstateSelected && (
            <>
              <SummaryItem
                ...
                value={
                  TYPE_LABEL[data.transactionType ?? ""] ?? data.transactionType
                }
              />
          ...
          {isEnergySelected && (
            <div className="col-span-2">
              <SummaryItem
                label="Energy Needs"
                value={data.energyNeedsDescription || "Not provided"}
              />
            </div>
          )}
